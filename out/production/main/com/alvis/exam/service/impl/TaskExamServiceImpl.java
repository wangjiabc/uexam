    ...
    public void edit(TaskRequestVM model, User user) {
        ...
        if (actionEnum == ActionEnum.ADD) {
            ...
            TextContent textContent = textContentService.jsonConvertInsert(model.getPaperItems(), now, p -> {
                ...
                taskItemObject.setExamPaperId(((ExamResponseVM) p).getId());
                taskItemObject.setExamPaperName(((ExamResponseVM) p).getName());
        ...
        //更新试卷的taskId
        List<Integer> paperIds = (List<Integer>) model.getPaperItems().stream().map(d -> ((ExamResponseVM) d).getId()).collect(Collectors.toList());
